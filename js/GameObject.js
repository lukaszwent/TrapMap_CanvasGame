...
class GameObject {
  constructor(updateStateMilliseconds, delay = 0) {
    ...
    this.updateStateMilliseconds =
      updateStateMilliseconds; /* change to suit the gameObject state update in milliseconds. Smaller numbers give a faster gameObject update. */
    this.delay = delay; /* delay the start of the updateState() method */
  }

  start() {
    if (
      this.updateStateMilliseconds !== null &&
    ...
    function startUpdateStateInterval() {
      // this function is only ever called from inside the start() method
      this.gameObjectInterval = setInterval(
